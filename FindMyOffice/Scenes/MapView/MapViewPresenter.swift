...
final class MapViewPresenter: MapViewPresentationLogic {
    ...
    func presentOfficesLocation(response: Offices.Fetch.Response) {
        ...
        response.offices.forEach {
            officePresent.append(Offices.Fetch.ViewModel.Office(name: $0.name,  rooms: String($0.rooms ?? 0),image: $0.image, id: $0.id ?? 0,location: Offices.Fetch.ViewModel.Location(latitude: $0.location?.latitude, longitude: $0.location?.longitude) ))
            }
